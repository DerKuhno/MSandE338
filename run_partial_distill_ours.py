"""
Continuous-noise UNDO variant.
Instead of noising once before distillation, noise is applied after every
optimizer step throughout training (do_continuous_corruption).

Launch Command: python run_partial_distill_ours.py --run_all
Single experiment: python run_partial_distill_ours.py --setup gemma-2-0.3B_MaxEnt --alpha 0.01 --beta 0.1
"""
...
# Continuous noise alphas: much smaller than one-shot UNDO because noise is
# applied every step. Start with a small sweep and widen once you see results.
SWEEP_ALPHAS = [0.001, 0.005, 0.01, 0.02, 0.05]
SWEEP_BETAS = [0.1]
SWEEP_SEEDS = [111, 222, 333, 444, 555]
NOISE_EVERY_N_STEPS = 1  # Apply noise after every optimizer step
OVERWRITE_OK = True
...
setups = {
    "gemma-2-0.3B_MaxEnt": {
        ...
        'output_dir'        : f"{MODEL_DIR}/partial_distill_models_arith/gemma-2-0.3B_MaxEnt-arithmetic-continuous_distill-alpha-beta",
        'cache_dir'         : CACHE_DIR,
        ...
        'use_wandb'        : True,
        'wandb_project'    : "gemma-2-0.3B_all_arithmetic+eng_continuous_distill",
        'wandb_run_name'   : None,
        'use_local_record' : True,
        'path_local_record': f"{MODEL_DIR}/local_records/partial_distill_models_arith/gemma-2-0.3B_MaxEnt-arithmetic-continuous_distill-alpha-beta.txt",

        'shrink_perturb_repeat'  : False,
        'noise_alpha'            : 0.0,   # one-shot noise disabled
        'noise_beta'             : 0.0,
    }
}
...
def run_experiment(setup_id, alpha, beta, seed=None,
                  ...
                  retain_arithmetic_threshold=RETAIN_ARITHMETIC_THRESHOLD,
                  forget_arithmetic_threshold=FORGET_ARITHMETIC_THRESHOLD):
    """Run a single continuous-noise experiment."""
    print(f"\n{'='*80}")
    print(f"LAUNCHING CONTINUOUS-NOISE EXPERIMENT:")
    print(f"  Setup: {setup_id}")
    print(f"  Continuous alpha: {alpha}  (applied every {NOISE_EVERY_N_STEPS} optimizer step(s))")
    print(f"  Beta: {beta}")
    if seed is not None:
        print(f"  Seed: {seed}")
    ...
    # one-shot noise stays 0; continuous noise uses the sweep alpha
    current_setup['continuous_noise_alpha'] = alpha
    current_setup['continuous_noise_beta'] = beta
    current_setup['noise_every_n_steps'] = NOISE_EVERY_N_STEPS
    ...
    partial_distill(
        ...
        noise_every_n_steps   = current_setup['noise_every_n_steps'],
        continuous_noise_alpha= current_setup['continuous_noise_alpha'],
        continuous_noise_beta = current_setup['continuous_noise_beta'],
...
if __name__ == "__main__":
    ...
    # If no valid arguments provided, show help
        ...
        parser.print_help()
        print("\nExamples:")
        print("  # Run a single continuous-noise experiment:")
        print("  python run_partial_distill_ours.py --setup gemma-2-0.3B_MaxEnt --alpha 0.01 --beta 0.1")
        print("  python run_partial_distill_ours.py --setup gemma-2-0.3B_MaxEnt --alpha 0.01 --seed 111")
        print("  # Run the full sweep in parallel:")
        print("  python run_partial_distill_ours.py --run_all")
        print("  # Run with a specific stopping condition:")
        print("  python run_partial_distill_ours.py --run_all --stop_condition retain_arithmetic_only")
        print("  # Resume from a specific alpha value:")
        print("  python run_partial_distill_ours.py --start_alpha 0.01")